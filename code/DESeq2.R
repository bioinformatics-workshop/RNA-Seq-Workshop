...
############## Differential Expression (END)##############


# ############## Post-Processing Assessment (START) ##############

# # LFC shrinkage for plotting
# resLFC <- lfcShrink(dds, coef=2, type="apeglm")
# res05LFC <- lfcShrink(dds, coef=2, type="apeglm", lfcThreshold = 1)

# # MA Plots

# ## LFC > 0, FDR < 0.1
# title = paste("MA-plot ( | LFC | > 0, FDR < 0.1)")

# pdf("analysis/deseq2/plots/MAplot-LFC0-FDR1.pdf", width = 7, height = 7)
# #plotMA(res, ylim=c(-2,2))
# plotMA(resLFC, ylim=c(-3,3),
#        main = title) # with shrunken log2 fold changes
# dev.off()

# png("analysis/deseq2/plots/MAplot-LFC0-FDR1.png")
# #plotMA(res, ylim=c(-2,2))
# plotMA(resLFC, ylim=c(-3,3),
#        main = title) # with shrunken log2 fold changes
# dev.off()
...
# ## LFC > 1, FDR < 0.05
# title = paste("MA-plot ( | LFC | > 1, FDR < 0.05)")

# pdf("analysis/deseq2/plots/MAplot-LFC1-FDR05.pdf", width = 7, height = 7)
# #plotMA(res, ylim=c(-2,2))
# plotMA(res05LFC, ylim=c(-3,3),
#        main = title) # with shrunken log2 fold changes
# dev.off()

# png("analysis/deseq2/plots/MAplot-LFC1-FDR05.png")
# #plotMA(res, ylim=c(-2,2))
# plotMA(res05LFC, ylim=c(-3,3),
#        main = title) # with shrunken log2 fold changes
# dev.off()

# ############## Post-Processing Assessment (END) ##############


# ############## Alternative DEG testing analysis ##############

# # Pairwise comparisons of groups
# levels = c("NC24", "CW24", "NC48", "CW48", "NC72", "CW72")
# dds$group <- factor(paste0(dds$treatment,dds$time), levels = levels)
# design(dds) <- ~ group

# deseq2_deg_v2 <- function(ref_sample, contrast_name){

# print(glue("Running ref_sample: {ref_sample} and contrast: {contrast_name}"))

# # set reference sample
# dds$group <- relevel(dds$group, ref = ref_sample)

# # run DESeq
# dds <- DESeq(dds)

# # Save results from specific contrast
# res <- results(dds, name=contrast_name)

# # subsetting results
# res_filt <- subset(res, padj < 0.1)
# up <- subset(res_filt, log2FoldChange > 0)
# dn <- subset(res_filt, log2FoldChange < 0)

# res05_filt <- subset(res, padj < 0.05)
# up_05 <- subset(res05_filt, log2FoldChange > 1)
# dn_05 <- subset(res05_filt, log2FoldChange < -1)

# # generates summary file
#   summary_file <- list("name" = contrast_name,
#                        "total_p1" = nrow(res_filt),
#                        "up_p1" = nrow(up),
#                        "dn_p1" = nrow(dn),
#                        "total_p05" = nrow(res05_filt),
#                        "up_p05" = nrow(up_05),
#                        "dn_p05" = nrow(dn_05))
...
#           left_join(., annot, c('geneid'='transcript'))
...
# ref_sample = c("NC24","NC48","NC72")
# contrast_name <- c("group_CW24_vs_NC24",
#                    "group_CW48_vs_NC48",
#                    "group_CW72_vs_NC72")

# sum_file <- map2(ref_sample, contrast_name, deseq2_deg_v2)

# sum_file_tbl <- sum_file %>%
# map_dfr(function(x) {x <- as.data.frame(x); x}) %>% 
#   as_tibble()
...
# # Full model (with all factors and conditions)
# # Reduced model (with factors removed)

# design(dds) <- ~treatment + time + treatment:time
# dds <- DESeq(dds, test = "LRT", reduced = ~treatment + time)

# # resultsNames(dds)
# # [1] "Intercept"          "treatment_CW_vs_NC" "time_48_vs_24"      "time_72_vs_24"      "treatmentCW.time48" "treatmentCW.time72"

# # Note: None of the contrast yielded significantly expressed genes with respect to time

# ### Comparisons NC vs CW

# deseq2_deg_v3 <- function(contrast_name){
...
# # Run function for comparison based on treatment
# summary_file <- deseq2_deg_v3("treatment_CW_vs_NC")

# # Combine summary tables for DEGs
# sum_file_tbl <- summary_file %>%
#   as_tibble() %>%
#   rbind(sum_file_tbl)
